...
const UnitList = () => {
    ...
    useEffect(() => {
        ...
        const fetchCustomKeywords = async () => {
            if (!currentUser) return;
            try {
                // Fix: Fetch from the correct collection
                const snap = await getDocs(collection(db, 'users', currentUser.uid, 'customKeywords'));
    ...
    // Get the display name for a keyword
    const getKeywordDisplay = keyword => {
        if (keyword.startsWith('custom:')) {
            ...
            return customKeyword ? customKeyword.name : `Custom:${customId}`;
        }
        return Keywords.getDisplayName ? Keywords.getDisplayName(keyword) : keyword;
    };
                                        ...
                                        {unit.keywords && unit.keywords.length > 0 ? (
                                            unit.keywords.map((keyword, index) => (
                                                <Badge
                                                    ...
                                                    className="keyword-badge me-1 mb-1"
                                                >
